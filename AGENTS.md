...
- Core logic: `src/llm.rs`, `src/config.rs`, `src/tool/` (8 tools + shared helpers in `src/tool.rs`).
- Tool descriptions live in `docs/tool/*.md`, compiled in via the `tool_description!` macro in `src/tool.rs`.
- LLM prompt templates live in `docs/prompt/*.md` (commit, message, style, summarize, translator), compiled in via the `prompt_description!` macro in `src/llm.rs`.
- Tool structs re-exported from `src/tool.rs`: `pub use <module>::<Name>`. Use as `crate::tool::{Read, List, …}`.
- Uses `rig-core` for LLM agent/tool orchestration (OpenAI-compatible API).

## Runtime Model
- All tools except `diff` read from the **staged git tree** (`git write-tree`), not the working tree. `staged_hash()` in `src/tool.rs` does `git rev-parse --show-toplevel` + `cd` + `git write-tree`, stored in a `OnceLock`.
- **`diff` is the exception**: it runs `git diff --cached` against the working index, not the tree hash. This means diff reflects what `git add` staged, while read/list/grep/glob/stat/log read from the frozen tree snapshot.
...
- See `gitconfig.example` for setup. The tool is designed to be set as `core.editor`.
...
## Tool Constraints (enforced by `paginate_output` in `src/tool.rs`)
...
- **diff**: capped at 50 KB (no line limit).
...
- `src/suffix.txt` — footer ("generated by LLM") not referenced in any code path.
